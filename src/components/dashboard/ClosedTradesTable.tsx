      ...
      action={
        <span
          className={`px-2.5 py-1 text-xs font-semibold rounded-md tabular-nums ${
            totalPnl >= 0 ? 'bg-emerald-50 text-emerald-700' : 'bg-rose-50 text-rose-700'
          }`}
        >
    ...
    >
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          <thead>
            <tr className="text-[10px] font-semibold uppercase tracking-[0.12em] text-slate-400 border-y border-slate-100">
          ...
          <tbody className="divide-y divide-slate-50">
            {visible.map((trade, index) => (
              ...
              >
                ...
                <td className="px-4 py-3 font-semibold text-slate-800">{trade.ticker}</td>
                ...
                <td className="px-6 py-3 text-right font-mono tabular-nums whitespace-nowrap">
                  <span
                    className={`font-semibold ${trade.pnlUSD >= 0 ? 'text-emerald-600' : 'text-rose-600'}`}
                  >
                    {isPrivate
                      ? '***'
