...
export function KPICard({
  ...
  return (
    <div
      className={`bg-white dark:bg-slate-900 px-5 py-4 rounded-xl border shadow-[0_1px_2px_rgba(15,23,42,0.04)] flex items-center gap-4 ${
        emphasis
    ...
    >
      ...
      <div className="w-full min-w-0">
        <div className="flex justify-between items-center gap-2">
          <p className="text-[10px] font-semibold uppercase tracking-[0.12em] text-slate-400 truncate dark:text-slate-500">
            {title}
          </p>
          {badge && (
            <span
        ...
        {hint && (
          <p className="text-[11px] text-slate-400 mt-0.5 truncate dark:text-slate-500">{hint}</p>
        )}
      </div>
    </div>
  );
